...
The name: **REAM**. As in, "that really reamed me."
...
becomes Rust: Gleam compiler (Rust) → `.beam` bytecode → REAM (Rust) →
...
trees. The supervisor pattern is a regular Gleam module that REAM
...
REAM exposes Rust functions to Gleam code as BIFs (Built-In Functions)
...
For REAM, the primary I/O patterns are:
...
Gleam's stdlib uses these for file operations and TCP. REAM would
...
REAM doesn't need to implement behaviours — they're Gleam library
...
The PoC proves the architecture with minimum surface area:

1. **Load a `.beam` file** produced by the Gleam compiler
2. **Execute a simple Gleam module** (fibonacci, list operations)
3. **Spawn processes** and pass messages between them
4. **Reduction-based scheduling** across multiple OS threads
5. **Register a Rust function** and call it from Gleam
6. **Per-process GC** triggered by allocation pressure

This validates: bytecode loading, term representation, the
interpreter loop, the scheduler, message passing, and native
function calls. Everything else is incremental.

**Estimated PoC size:** 5-10k lines of Rust.
**Estimated PoC time:** 2-3 weeks for someone focused.
...
A Gleam workflow running on REAM:
