...
- Gap closure from verification failures (`--gaps` mode)
...
The orchestrator provides user decisions in `<user_decisions>` tags (captured before planning).
...
For niche domains (3D, games, audio, shaders, ML), suggest running a dedicated research pass before planning.
...
Load planning context from the prompt. Extract: phase, phase directory, whether research and context files exist.

Read STATE.md if present for position, decisions, and blockers.
</step>
...
List all SUMMARY.md files in the phases directory, reading their frontmatter for: `affects`, `provides`, `tech_stack`, `decisions`, `patterns`.
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # User decisions captured before planning
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From prior research pass
...
Validate each created PLAN.md by reading and checking:
...
- Each `<task>` must have: `<name>`, `<files>` (if auto), `<action>`, `<verify>`, `<done>`
- If errors: Fix before committing (e.g., missing `<name>` → add it, checkpoint without `autonomous: false` → fix it)
</step>
...
```bash
git add phases/$PHASE-*/$PHASE-*-PLAN.md ROADMAP.md
...
Execute the phase plans in wave order.
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to execute the gap closure plans next
