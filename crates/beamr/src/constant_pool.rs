...
impl ConstantPool {
    ...
    fn push_block(&mut self, words: Vec<u64>) -> Result<usize, LoadError> {
        if words.is_empty() {
...
fn materialise_literal(
    ...
    match literal {
        ...
        Literal::Float(value) => {
            let block = pool.push_block(vec![0; 2])?;
        ...
        Literal::BigInteger(bytes) => {
            ...
            let block = pool.push_block(vec![0; 3 + limbs.len()])?;
            let term =
        ...
        Literal::Binary(bytes) | Literal::String(bytes) => {
            let block = pool.push_block(vec![0; 2 + packed_word_count(bytes.len())])?;
        ...
        Literal::Tuple(elements) => {
            ...
            let block = pool.push_block(vec![0; 1 + terms.len()])?;
        ...
        Literal::List(elements, tail) => {
            ...
            let block = pool.push_block(vec![0; elements.len() * 2])?;
        ...
        Literal::Map(entries) => {
            ...
            let block = pool.push_block(vec![0; 2 + keys.len() + values.len()])?;
            let term =
...
mod tests {
    ...
    #[test]
    fn cloned_pool_rebases_list_roots_to_cloned_blocks() {
        ...
        let original = pool.get(0).expect("original list");
        let copied = cloned.get(0).expect("cloned list");

        assert_ne!(original.heap_ptr(), copied.heap_ptr());
        let cons = Cons::new(copied).expect("cloned cons");
        assert!(Tuple::new(cons.head()).is_some());
    }
