...
use crate::term::boxed::{BoxedHeader, BoxedTag, Closure, Map, ProcBin, write_float, write_map};
...
/// Creates an assoc-updated map from `source` and raw key/value pairs.
///
/// Returns zero for non-map sources, malformed ABI data, or allocation failure;
/// generated code branches to the BEAM fail label for all zero returns.
...
/// Creates an exact-updated map from `source` and raw key/value pairs.
///
/// Returns zero when `source` is not a map, an update key is absent, the pair ABI
/// is malformed, or allocation fails. The source map is never mutated.
...
/// Reads a map key using a two-result ABI: status 1 carries a value, status 0 is missing.
...
/// Returns 1 when `map` contains `key`, otherwise 0.
