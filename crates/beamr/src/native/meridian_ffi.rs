...
pub fn register_meridian_ffi(
    ...
    registry.register(module, atom_table.intern("run_step_norn"), 4, nif_run_step_norn)?;
    Ok(())
}
...
fn ok_binary(content: &[u8]) -> Result<Term, Term> {
    ...
    write_tuple(tuple_heap, &[Term::atom(Atom::OK), binary])
        .ok_or(Term::atom(Atom::ERROR))
}
